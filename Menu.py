                ...
                if dataIsUpdated()==False and MegaStorage.connectionError==False:
                    ...
                    if len(OutStandingShare)>=numberOfCompanies:
                        ...
                        if len(shareSupportLevel3)>=numberOfCompanies and len(shareReportDate)>=numberOfCompanies:
                            MegaStorage.dataAsOf = MegaStorage.shareDataAsOf[numberOfCompanies-1]
...
def programLoader():
    ...
    if allFilesArePresent()==True and filesAreEmpty()==False:
        ...
        if response=='Y' or response=='y':
            ...
            message1 = "\t\t[ Status ] :\tData has been loaded successfully from file\n"
            message2 = "\t\t[ Error ] :\tUnable to load from file\n"
            if dataLoader(message1, message2)==True:
                menu()
    else:
        if filesAreEmpty()==True:
            print("\t\t[ Error ] :\tUnable to retrieve required data (Empty files)")
        else:
            print("\t\t[ Error ] :\tUnable to locate required files (Missing files)")
        print("\n\t-------------------------------------------------------------------------------------------------------------------\n")
        response = input("\t\tReset Files (Y/N):\t")
        print("\n\t-------------------------------------------------------------------------------------------------------------------\n")
        if response=='Y' or response=='y':
            ...
            if isConnected()==True:
                ...
                if response=="Y" or response=="y":
                    ...
                    print("\t\t[ Error ] :\tProgram cannot run with missing data\n")
                    Functions.programExit()
            else:
                print("\t\t[ Error ] :\tUnable to update data (System not online)\n")
                Functions.programExit()
        else:
            print("\t\t[ Error ] :\tProgram cannot run with missing data\n")
            Functions.programExit()
