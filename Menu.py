...
def commandLine():
    ...
    # Generate stock report
        ...
        if userCommandFiltered.find("?")>=0:
            ...
            userCommandFiltered = removeUnwantedCharacter(userCommand.lower())
            if userCommandFiltered.find("simple")>=0 or userCommandFiltered.find("single")>=0 or userCommandFiltered.find(".")>=0:
                userCommand = input("\t\tMethod (simple / advance)>\t")
                userCommandFiltered = removeUnwantedCharacter(userCommand.lower())
                if userCommandFiltered.find("simple")>=0 or userCommandFiltered.find("advance")>=0:
                    method = userCommandFiltered
                    userCommand = input("\t\tCompany>\t")
                    userCommandFiltered = removeUnwantedCharacter(userCommand.upper())
                    for index in range(0, numberOfCompanies):
                        if userCommandFiltered.find(MegaStorage.Symbol[index])>=0:
                            ...
                            if method.find("simple")>=0:
                                ...
                                if len(MegaStorage.companyVolume)>0:
                                    Functions.generateLogicalSingleCompanyReport(index)
                                else:
                                    print("\n\t-------------------------------------------------------------------------------------------------------------------")
                    ...
                    print("\n\t-------------------------------------------------------------------------------------------------------------------")
                    print("\t\t[ Error ] :" + Functions.center("Invalid Method", 60))
            elif userCommandFiltered.find("advance")>=0 or userCommandFiltered.find("multiple")>=0 or userCommandFiltered.find("*")>=0:
                Functions.generateAdvanceCompanyReport()
            else:
                print("\n\t-------------------------------------------------------------------------------------------------------------------")
                print("\n\t\t[ Error ] :" + Functions.center("Invalid Type", 60) + "\n")
        commandLine()
