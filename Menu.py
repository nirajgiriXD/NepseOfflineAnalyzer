...
def commandLine():
    ...
    # Clear Screen
    elif userCommandFiltered.find("cls") or userCommandFiltered.find("clear"):
        os.system('cls')
        menu()
            ...
            if factor.find("pe")>=0 or factor.find("eps")>=0 or factor.find("price")>=0:
                ...
                for index in range(0, len(SectorList1)):
                    if MegaStorage.SectorList1[index].find(userCommandFiltered)>=0:
                        Functions.sortData(index, factor, arr2, arr3)
                        sortError = False
                        break
                if sortError==True:
                    print("\n\t-------------------------------------------------------------------------------------------------------------------")
                    ...
                    try:
                        if userCommandFiltered=="price" or userCommandFiltered=="eps" or userCommandFiltered=="pe":
                            ...
                            if userCommandFiltered.find("price")>=0:    
                                Functions.rangeOfData(index, "price", rangeLowerLimit, rangeUpperLimit)
                            elif userCommandFiltered.find("eps")>=0:
                                Functions.rangeOfData(index, "eps", rangeLowerLimit, rangeUpperLimit)
                            else:
                                Functions.rangeOfData(index, "pe", rangeLowerLimit, rangeUpperLimit)
                        else:
                            print("\n\t-------------------------------------------------------------------------------------------------------------------")
