...
def commandLine():
    ...
    # Generate stock report
        ...
        if userCommandFiltered.find("?")>=0:
            ...
            userCommand = input("\n\t\tMethod>\t")
            userCommandFiltered = removeUnwantedCharacter(userCommand.lower())
            if userCommandFiltered.find("ai")>=0 or userCommandFiltered.find("advance")>=0:
                Functions.analyzePattern()
            elif userCommandFiltered.find("simple")>=0 or userCommandFiltered.find("single")>=0:
                userCommand = input("\t\tCompany Symbol>\t")
                userCommandFiltered = removeUnwantedCharacter(userCommand.upper())
                for index in range(0, numberOfCompanies):
                    if userCommandFiltered.find(Symbol[index])>=0:
                        Functions.generateReport(index)
                        companyError = False
                        break
...
def menu():
    ...
    print("\t\t9. Use 'summary' to get market overview")
    print("\t\t10. Use 'reset' to reset all data and files")
    print("\t\t11. Use 'clear' to clear the screen")
    print("\t\t12. Use 'quit' to exit program")
    print("")
    commandLine()
