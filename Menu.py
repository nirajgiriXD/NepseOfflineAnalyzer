...
def commandLine():
    ...
    # Data Ectract Option
        ...
        if userCommandFiltered.find("?")>=0:
            ...
            print("\n\t-------------------------------------------------------------------------------------------------------------------")
            print("\n\t\t[ Status ] :\tChecking a few things...", end='\r')
            if isConnected()==True:
                if dataIsUpdated()==False:
                    ...
                    if len(shareSupportLevel3)>=numberOfCompanies and len(shareReportData)>=numberOfCompanies:
                        ...
                        print("\t\t[ Error ] :\tData extraction unsuccessful\n")
                        marketDataLoader()
                        if len(OutStandingShare)>=numberOfCompanies:
                            reportDataLoader()
                        if len(shareSupportLevel3)>=numberOfCompanies and len(shareReportDate)>=numberOfCompanies:
                            print("\t\t[ Status ] :\tData loaded successfully from file.")
                        else:
                            print("\t\t[ Error ] :\tFailed to load data\n")
                            Functions.programExit()
                else:
                    print("\t\t[ Message ] :\tData is up to date; no need to extract\n")
            else:
                print("\t\t[ Warning ] :\tDevice is not connected to the internet\n")
        commandLine()
...
def programLoader():
    ...
    if isConnected()==True:
        if dataIsUpdated()==False:
            print("\t\t[ Status ] :\tData is not updated")
            print("\n\t-------------------------------------------------------------------------------------------------------------------")
            response = input("\n\t\tUpdate Data (Y/N):\t")
            print("\n\t-------------------------------------------------------------------------------------------------------------------\n")
            ...
            if len(shareSupportLevel3)>=numberOfCompanies and len(shareReportDate)>=numberOfCompanies:
                # os.system('cls')
                print("\t\t[ Status ] :\tData is up to date and has been loaded successfully from file\n")
                menu()
            else:
                print("\t\t[ Error ] :\tData is up to date but unable to load from file\n")
                Functions.programExit()
    else:
        marketDataLoader()
        if len(OutStandingShare)>=numberOfCompanies:
        ...
        if len(shareSupportLevel3)>=numberOfCompanies and len(shareReportDate)>=numberOfCompanies:
            # os.system('cls')
            print("\t\t[ Status ] :\tData loaded successfully in offline mode\n")
            menu()
        else:
            print("\t\t[ Error ] :\tFailed to load data in offline mode\n")
            Functions.programExit()
