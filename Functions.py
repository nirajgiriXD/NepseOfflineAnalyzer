...
def sectorDisplay():
    sectorLength = len(MegaStorage.SectorList2) - 1
    nepseAvg = 0.0
    str = "Nepse"
    print("\n\t===================================================================================================================")
    print(f"{MegaStorage.bold}" + center("Sector Analysis", 116) + f"{MegaStorage.normal}")
    print("\t===================================================================================================================\n")
    for index1 in range(0, sectorLength):
        count = 0
        secAvg = 0.0
        for index2 in range(0, MegaStorage.numberOfCompanies):
            if MegaStorage.SectorList2[index1]==MegaStorage.Sector[index2]:
                secAvg = secAvg + MegaStorage.PercentageChange[index2]
                count += 1
        try:
            secAvg = secAvg / count
        except:
            secAvg = 0.0
        nepseAvg = nepseAvg + secAvg
        strSecAvg = "{:.2f}".format(secAvg)
        print(f"\t\t{MegaStorage.SectorList2[index1].ljust(29)}:\t{strSecAvg} %")
    nepseAvg = nepseAvg / sectorLength
    strNepseAvg = "{:.2f}".format(nepseAvg)
    print(f"\t\t{str.ljust(29)}:\t{strNepseAvg} %")
    print("")
                ...
                if rangeFlag==True:
                    for count in range(0, lenArr):
                        ...
                        if MegaStorage.OutStandingShare[index]>=rangeLowerLimit and MegaStorage.OutStandingShare[index]<=rangeUpperLimit:
                            print(f"\t\t{str(MegaStorage.Symbol[index]).ljust(17)}{str(MegaStorage.OutStandingShare[index]).ljust(23)}{str(MegaStorage.Eps[index]).ljust(16)}{str(MegaStorage.PERatio[index]).ljust(16)}{str(MegaStorage.MarketPrice[index]).ljust(19)}{str(MegaStorage.share52WeeksHigh[index]).ljust(23)}{MegaStorage.share52WeeksLow[index]}")
                else:
                    for count in range(0, lenArr):
                        index = MegaStorage.arr3[count]
                        print(f"\t\t{str(MegaStorage.Symbol[index]).ljust(17)}{str(MegaStorage.OutStandingShare[index]).ljust(23)}{str(MegaStorage.Eps[index]).ljust(16)}{str(MegaStorage.PERatio[index]).ljust(16)}{str(MegaStorage.MarketPrice[index]).ljust(19)}{str(MegaStorage.share52WeeksHigh[index]).ljust(23)}{MegaStorage.share52WeeksLow[index]}")
...
def projectileSummary(bullPer):
    if bullPer>=0 and bullPer<20:
        str =  "Strong Bearish Sentiment ( NEPSE >= 1 % (Red Marubozu))"
    elif bullPer>=20 and bullPer<40:
        str = "Bearish Sentiment ( NEPSE >= 0.5 % (Red Hammer))"
    elif bullPer>=40 and bullPer<50:
        str = "Sideways (Slightly Bearish Sentiment ( ~ NEPSE ~ (Red Doji)))"
    elif bullPer>= 50 and bullPer<60:
        str = "Sideways (Slightly Bullish Sentiment ( ~ NEPSE ~ (Green Doji)))"
    elif bullPer>=60 and bullPer<80:
        str = "Bullish Sentiment ( NEPSE >= 0.5 % (Green Hammer))"
    elif bullPer>=80 and bullPer<=100:
        str = "Strong Bullish Sentiment ( NEPSE >= 1 % (Green Marubozu))"
    else:
        str = "Indecisive (Unable to Predict)"
    return str
...
def generateAutomatedReport(disFilter):
    ...
    # Patterns: Hammer, Doji, MorningStar, EveningStar, DragonFly, GraveStone, EngulfingBullish, EngulfingBearish
    ...
    if sizeArr>0:
        ...
        title2 = ["Data Date", "Bull Count", "Bear Count", "Indecisive Count", "Bull Percentage", "Bear Percentage", "Projectile Status"]
        print("\n\t===================================================================================================================")
        if disFilter=="NULL":
            print(center(f"{MegaStorage.bold}REPORT{MegaStorage.normal}", 116))
        else:
            ...
            if MegaStorage.bullPoint[index]>0 or MegaStorage.bearPoint[index]>0:
                if disFilter=="BULL":
                    ...
                    if bullTotal==0 and bearTotal==0:
                        ...
                        bbPer = (bullTotal / (bullTotal + bearTotal)) * 100
                        print(f"\t\t{title2[0].ljust(21)}:\t{MegaStorage.dataAsOf}")
                        print(f"\t\t{title2[1].ljust(21)}:\t{MegaStorage.green}{bullTotal}{MegaStorage.normal}")
                        print(f"\t\t{title2[2].ljust(21)}:\t{MegaStorage.red}{bearTotal}{MegaStorage.normal}")
                        print(f"\t\t{title2[3].ljust(21)}:\t{MegaStorage.yellow}{indecisiveTotal}{MegaStorage.normal}")
                        print(f"\t\t{title2[4].ljust(21)}:\t{MegaStorage.green}" + str("{:.2f}".format(bbPer)) + f"{MegaStorage.normal} %")
                        print(f"\t\t{title2[5].ljust(21)}:\t{MegaStorage.red}" + str("{:.2f}".format(100 - bbPer)) + f"{MegaStorage.normal} %")
                        print(f"\t\t{title2[6].ljust(21)}:\t{projectileSummary(bbPer)}")
                    break
        print("")
    else:
        print("\n\t-------------------------------------------------------------------------------------------------------------------")
