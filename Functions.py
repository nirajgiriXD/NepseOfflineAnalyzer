...
def sortData(index, factor, rangeCall):
    sortFlag = False
...
def rangeDisplay(displayCall):
    if len(MegaStorage.rangeCompanyIndex)>0:
        ...
        for count in range(0, len(MegaStorage.rangeCompanyIndex)):
            ...
            if displayCall=="eps":
                print(f"\t\t{MegaStorage.Symbol[index]}\t\t\t{str(MegaStorage.Eps[index]).ljust(24)}{str(MegaStorage.PERatio[index]).ljust(24)}{str(MegaStorage.Pbv[index]).ljust(24)}{str(MegaStorage.MarketPrice[index])}")
            elif displayCall=="price":
                print(f"\t\t{MegaStorage.Symbol[index]}\t\t\t{str(MegaStorage.MarketPrice[index]).ljust(24)}{str(MegaStorage.PERatio[index]).ljust(24)}{str(MegaStorage.Pbv[index]).ljust(24)}{MegaStorage.Eps[index]}")
            else:
                print(f"\t\t{MegaStorage.Symbol[index]}\t\t\t{str(MegaStorage.PERatio[index]).ljust(24)}{str(MegaStorage.Pbv[index]).ljust(24)}{str(MegaStorage.Eps[index]).ljust(24)}{str(MegaStorage.MarketPrice[index])}")
        print("")
    else:
        print("\n\t-------------------------------------------------------------------------------------------------------------------")
